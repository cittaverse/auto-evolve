# Lessons Learned - Hulk 🟢

> **原则**：精炼、通用、可内化  
> **更新**：每次任务后补充 1-2 条

---

## 核心原则

### 1. 先扫描，再执行

**场景**：研究、专利、论文、竞品分析  
**错误**：执行后发现 prior art，被迫返工  
**内化**：任何输出前，先问"外部证据扫描了吗？"

---

### 2. 方向确认 > 执行速度

**场景**：专利权利要求、产品定位、技术路线  
**错误**：假设自己理解正确，事后返工  
**内化**：方向性问题，花 5 分钟确认，省 5 小时返工

---

### 3. 验证环境，再选方案

**场景**：技术方案选择（Python 库、CLI 工具、在线服务）  
**错误**：假设环境支持，执行时报错  
**内化**：`which xxx` / `import xxx` 是第一步

---

### 4. 版本备份，再修改

**场景**：任何文档修改  
**错误**：直接覆盖，无法追溯  
**内化**：`cp file.md file_v1.md` 是肌肉记忆

---

### 5. 外部依赖，主动追踪

**场景**：需要 V 执行的任务（外部联系、决策）  
**错误**：建议后不追踪，任务卡住  
**内化**：每日询问一次，连续 3 天无回复则升级

---

### 6. 🔴 密钥绝不上传（2026-03-13 新增）

**场景**：代码提交、GitHub 推送  
**错误**：`citta_web_demo.html` 包含 Google API Key，被 Google 安全团队检测并通知  
**后果**：
- Google 发送 abuse notification
- API key 需立即重置
- Git 历史需清理（或 force push）

**内化**：
```
提交前检查清单：
1. 是否有 API key/密码/token？
2. 是否有 .env 文件？
3. 是否有配置文件包含敏感信息？

防护措施：
1. .gitignore 添加 *.html, *.env, *config*.json
2. 使用环境变量：os.environ.get('API_KEY')
3. 使用 pre-commit hook 检测密钥
```

**事故时间线**（2026-03-13）：
| 时间（UTC） | 事件 |
|-------------|------|
| 11:03 | 收到 Google abuse notification |
| 11:05 | 删除泄露文件 `citta_web_demo.html` |
| 11:06 | 发现更多泄露：`scripts/install-cron.sh` 包含 GitHub Token |
| 11:07 | 删除所有硬编码 token |
| 11:08 | 添加 .gitignore |
| 11:09 | 添加 pre-commit hook |
| 11:10 | 全部推送完成 |

**防护升级**：
- ✅ `.gitignore` 添加 `*.html`, `*.env`, `*config*.json`
- ✅ Pre-commit hook 自动检测 6 种密钥模式
- ✅ 全仓库扫描，无其他泄露

---

## 决策树

```
任务开始
  ↓
是否需要外部证据？→ 是 → 先扫描（学术 + 专利 + 竞品）
  ↓
是否方向性决策？→ 是 → 先确认 V
  ↓
是否需要特定工具？→ 是 → 先验证环境
  ↓
执行
  ↓
是否修改现有文件？→ 是 → 先备份 v1
  ↓
是否包含密钥？→ 是 → 立即停止，使用环境变量
  ↓
完成 → 主动汇报 + 更新 Notion
```

---

## 错误模式 → 条件反射

| 错误模式 | 条件反射 |
|----------|----------|
| 未扫描 prior art | "证据在哪？" |
| 未确认方向 | "V 确认了吗？" |
| 未验证环境 | "which xxx?" |
| 直接覆盖文件 | "v1 备份了吗？" |
| 不追踪外部依赖 | "今天问了吗？" |
| **提交代码** | **"有 API key 吗？"** |

---

## 自我检测

每次任务后问自己：
1. 扫描了吗？
2. 确认了吗？
3. 验证了吗？
4. 备份了吗？
5. 追踪了吗？
6. **有密钥吗？**

**3 个以上"否"** → 需要复盘

---

*最后更新：2026-03-13 11:10 UTC*  
*来源：Phase 1-3 复盘 + API Key 泄露事故*

---

### 7. 🔴 先验证再报告（2026-03-16 新增）

**场景**：任何状态报告、进展汇报  
**错误**：
- 假设 API Key 缺失（>96h），实际已配置
- 报告 GEO 完成，实际 commits 未推送

**教训**：
- 不要假设，要验证
- 明确区分"已记录"和"已执行"
- 不能做的事直接说明原因

**内化**：
```bash
报告前验证清单：
1. API Keys: printenv | grep DASHSCOPE
2. Git commits: git log -1 --oneline
3. Cron 状态：openclaw cron list
4. 文件存在：ls -la
5. 测试运行：python3 test_*.py

状态分级：
- ✅ 已验证：已运行验证命令
- ⚠️ 已记录：日志已写，待验证
- ❌ 假设：禁止使用
```
